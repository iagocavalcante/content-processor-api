...
export class SQLiteAdapter extends BaseAdapter {
    ...
    `);
        ...
        const applyMigration = this.db.transaction((migration) => {
            ...
            this.db.prepare('INSERT INTO izi_migrations (version, name) VALUES (?, ?)').run(migration.version, migration.name);
        });
    ...
    async rollback(targetVersion = 0) {
        ...
        const rows = stmt.all(targetVersion);
        const rollbackMigration = this.db.transaction((version) => {
    ...
    async getMigrationStatus() {
        ...
        const applied = new Map(stmt.all().map(r => [r.version, r.name]));
    ...
    async insertJob(job) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        const result = stmt.run(job.state, job.queue, job.worker, JSON.stringify(job.args), JSON.stringify(job.meta), JSON.stringify(job.tags), JSON.stringify(job.errors), job.attempt, job.maxAttempts, job.priority, job.scheduledAt.toISOString());
    ...
    async fetchJobs(queue, limit) {
        ...
        LIMIT ?
      `);
            const rows = selectStmt.all(queue, limit);
            if (rows.length === 0)
                return [];
    ...
    async updateJob(id, updates) {
      ...
      WHERE id = ?
    `);
        stmt.run(updates.state ?? null, updates.errors ? JSON.stringify(updates.errors) : null, updates.completedAt?.toISOString() ?? null, updates.discardedAt?.toISOString() ?? null, updates.cancelledAt?.toISOString() ?? null, updates.scheduledAt?.toISOString() ?? null, updates.meta ? JSON.stringify(updates.meta) : null, id);
        const getStmt = this.db.prepare('SELECT * FROM izi_jobs WHERE id = ?');
